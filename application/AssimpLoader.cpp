...
Object *AssimpLoader::load(const std::string &path)
{
    ...
    process(scene, scene->mRootNode, rootNode, rootPath);
    return rootNode;
}

void AssimpLoader::process(const aiScene *scene, aiNode *aiNode, Object *parent, const std::string &prefix)
{
    ...
    for (int i = 0; i < aiNode->mNumMeshes; i++)
    {
        ...
        // 解析得到Mesh
        auto mesh = processMesh(scene, aiMesh, prefix);
        // 将Mesh 添加到Node子节点上
        node->addChild(mesh);
    }
    ...
    {
        process(scene, aiNode->mChildren[i], node, prefix);
    }
}
...
Mesh *AssimpLoader::processMesh(const aiScene *scene, aiMesh *aiMesh, const std::string &prefix)
{
	...
	if (aiMesh->mMaterialIndex >= 0) {
		...
		texture = processTexture(scene, aiMat, aiTextureType_DIFFUSE, prefix);
		if (texture == nullptr) {
...
Texture *AssimpLoader::processTexture(const aiScene *scene, const aiMaterial *aiMat,
